...
# create a new data frame containing time between release and parole eligibility
# data for 2020, filtered by Hispanic, White, and Black races, with additional
# columns for overall release categorization, count, proportion, and tooltip info
...
# get list of states where data is available
states <- unique(ncrp_time_between_release_ped_2020_by_race$state)

# create bar graphs by state
...
# ncrp_sentlgth_timesrvd_rel created in tab_releases_from_prison.R
ncrp_pp_model_data <- ncrp_sentlgth_timesrvd_rel %>%

  # remove offenses, releases, admissions, race that were classified as "other"
  ...
  # # factor variables
  # mutate(release_within_1yr_ped = as.numeric(release_within_1yr_ped),
  #        state      = factor(state),
  #        sex        = factor(sex),
  #        race       = factor(race),
  ...
  # merge with info on abolishing parole
  left_join(parole_info_by_state, by = "state") %>%
...
# subset to 2020 data for test data and the rest of the years for training
train = Smarket %>%
  filter(Year < 2005)
