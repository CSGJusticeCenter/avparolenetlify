...
fnc_create_scatter_charts_by_state <- function(df, group_var, measure, source = ncrp_csg_source) {
  ...
  all_charts <- purrr::map(.x = states, .f = function(state_name) {
    ...
    # # Desired order for fbi_index levels
    # desired_order <- rev(c(
    ...
    # ))
    #
    # # Reorder fbi_index levels and sort data
    # df1 <- df1 %>%
    #   mutate(fbi_index = factor(fbi_index, levels = desired_order, ordered = TRUE)) %>%
    #   arrange(fbi_index, !!sym(group_var))
    #
    # # Create y-axis labels in the desired order
...
fnc_create_icons_homepage <- function(rri_raw, rri_digits = 1, fillcolor = "darkgray", partialcolor = "white",
  ...
  first_icon_opts <- fnc_icon_options(partialval = 0, empty = emptycolor, fill = first_icon_color, partial = first_icon_color, fillHoriz = fillHoriz)
