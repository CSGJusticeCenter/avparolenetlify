...
cross_tab_data <- ncrp_yearendpop %>%
  filter(!is.na(sentlgth) &
           !is.na(offgeneral)) %>%
  mutate(offgeneral = case_when(offgeneral == "Other or Unknown" ~ "Other or Unspecified",
                                TRUE ~ offgeneral),
         offdetail  = case_when(offdetail == " Other or Unknown" ~ "Other or Unspecified",
                                TRUE ~ offdetail),
...
# cross tab by state - missingness for 1-25 years
all_sentlgth_125_ped <- cross_tab_data %>%
  group_by(state) %>%
  mutate(sentlgth_category = case_when(
    sentlgth == "1-1.9 years" ~ "1-24.9 years",
    sentlgth == "2-4.9 years" ~ "1-24.9 years",
    sentlgth == "5-9.9 years" ~ "1-24.9 years",
    sentlgth == "10-24.9 years" ~ "1-24.9 years",
    TRUE ~ sentlgth
  )) %>%
  filter(sentlgth_category == "1-24.9 years") %>%
  count(sentlgth_category, parelig_status) %>%
  pivot_wider(names_from = parelig_status, values_from = n, values_fill = 0) %>%
  clean_names() %>%
  mutate(total =
           missing +
  ...
  mutate_at(vars(missing:future_6_years), list(percent = ~ (. / total))) %>%
  mutate(state = as.factor(state),
         sentlgth_category = as.factor(sentlgth_category))
...
for (folder in theseFOLDERS){
  ...
  save(all_sentlgth_125_ped ,   file = file.path(folder, "all_sentlgth_125_ped.rds"))
